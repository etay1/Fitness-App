...
const UserWeightForm = ({ closeAddUserWeightPopup }) => {
	const { successMessage, updateSuccessMessage } = useSuccessMessage();

	const { isSuccess, handleInsertion } =
		useUserWeightForm(updateSuccessMessage);

	const { validationSchema, key } =
		useUserWeightValidationSchema(updateSuccessMessage);
