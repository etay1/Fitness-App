        ...
        if (category === "strength") {
            setValidationSchema(
                Yup.object().shape({
                    ...
                    startTime: Yup.startTime().required("Start Time is required"),
                    endTime: Yup().endTime().required("End Time is required"),
                    sets: Yup.number().required.min(1, "Sets must be greater than 0"),
                    repsPerSet: Yup.number().required.min(1, "Reps must be greater than 0"),
                })
            );
